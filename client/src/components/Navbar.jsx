      ...
      <div className="container mx-auto flex flex-col lg:flex-row lg:justify-between items-center bg">
        <div className="flex justify-between w-full lg:w-auto items-center mb-4 lg:mb-0 bg-gray-100 rounded-lg hover:scale-110 transition-transform duration-200">
          <Link to="/" className="flex items-center">
            <img src={logo} alt="Logo" className="w-40 h-30 animate-cart-slide-in " />
          </Link>
        ...
        <ul className={`${isMenuOpen ? "block" : "hidden"} lg:flex flex-col lg:flex-row gap-6 w-full lg:w-auto bg-white lg:bg-transparent p-4 lg:p-0  text-slate-800  lg:static absolute top-20 left-0 z-50 font-extrabold font-poppins italic `}>
        ...
        <div className="hidden lg:flex items-center gap-6 text-slate-800  relative">
          <Link to="/wishlist">
            <FaHeart size={24} className="cursor-pointer hover:text-blue-600 hover:scale-110 transition-transform duration-200" />
          </Link>
          <Link to="/cart">
            <div className="relative">
              <FaShoppingCart size={24} className="cursor-pointer hover:text-blue-600 hover:scale-110 transition-transform duration-200" />
              {cartItemCount > 0 && (
  ...
  {userName && <span className="text-sm font-semibold">{userName}</span>}
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-40 bg-white shadow-lg rounded-lg py-2 z-50 animate__animated animate__fadeIn animate__faster">
                <p className="px-4 py-2 font-bold">Coins: {walletCoins} 🪙</p> {/* Display wallet coins */}
                {userName ? (
                  <>
                    <Link
                      to="/account"
