...
const AddVoucher = ({ onVoucherAdded, setMessage }) => {
    ...
    const addVoucher = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch('http://localhost:3000/api/admin/vouchers/addVoucher', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    maxUsage: newVoucher.maxUsage === '' ? null : Number(newVoucher.maxUsage),
                    usageLimitPerUser: newVoucher.maxUsage === '' ? 1 : Number(newVoucher.usageLimitPerUser),
            ...
            if (data.success) {
                ...
                setMessage({ type: 'error', text: data.message || 'Failed to create voucher' });
            }
        } catch (error) {
    ...
    return (
        <section className="bg-white rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label className="block text-gray-600 mb-1" htmlFor="usageLimitPerUser">
            ...
            <button
                onClick={addVoucher}
                disabled={loading || !newVoucher.code || !newVoucher.discount || !newVoucher.expiryDate}
                className={`mt-6 w-full flex items-center justify-center bg-blue-600 text-white px-6 py-3 rounded-md shadow hover:bg-blue-700 transition duration-300 ${loading || !newVoucher.code || !newVoucher.discount || !newVoucher.expiryDate ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
            >
