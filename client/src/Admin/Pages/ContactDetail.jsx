...
const ContactUsEnquiry = () => {
  ...
  const confirmDeleteContact = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/contact/deleteContact/${contactToDelete._id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <ConfirmModal
        ...
        message={contactToDelete ? `Are you sure you want to delete "${contactToDelete.name}"?` : ""}
        confirmText="Delete"
        cancelText="Cancel"
      />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px' }}>
          ...
          <tbody>
            {currentContacts.map((contact, index) => (
              <tr key={contact._id}>
                <td style={cellStyle('60px')} title={indexOfFirstItem + index + 1}>
