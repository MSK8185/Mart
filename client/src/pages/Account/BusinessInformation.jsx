...
export default function BusinessInfoForm() {
        ...
        const fetchVerificationData = async () => {
          try {
            const response = await axios.get(
              `http://20.40.59.234:3000/api/business/getBusinessInfo/${email}`,
              {
                headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !allowedTypes.includes(files.panFile.type) ||
      !allowedTypes.includes(files.aadharFile.type)
    ) {
    ...
    if (
      files.panFile.size > maxFileSize ||
      files.aadharFile.size > maxFileSize
    ) {
    ...
    try {
      const response = await axios.post(
        "http://20.40.59.234:3000/api/business",
        data,
        {
          headers: {
      ...
      return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {files.panFile && (
              <div className="mt-2 p-2 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-700">
                  ✓ Selected:{" "}
                  <span className="font-medium">{files.panFile.name}</span>
                </p>
            ...
            {files.aadharFile && (
              <div className="mt-2 p-2 bg-green-50 rounded-lg">
                <p className="text-sm text-green-700">
                  ✓ Selected:{" "}
                  <span className="font-medium">{files.aadharFile.name}</span>
                </p>
          ...
          <button
            ...
            className={`w-full py-3 px-4 rounded-xl transition duration-300 font-medium ${
